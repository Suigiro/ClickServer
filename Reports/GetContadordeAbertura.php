...
namespace Piwik\Plugins\ClickServer\Reports;
 ...
 */
class GetContadordeAbertura extends Base
{
    protected function init()
    {
        parent::init();

        $this->name          = Piwik::translate('ClickServer_ContadordeAbertura');
        $this->dimension     = null;
        $this->documentation = Piwik::translate('Registro de Aberturas de emails de uma campanha');
        ...
        $this->metrics = array('campaings', 'data', 'opens', 'clicks');
        ...
        // If a menu title is specified, the report will be displayed in the menu
            $this->menuTitle    = 'ClickServer_ContadordeAbertura';
        ...
        // exported as a widget
             $this->widgetTitle  = 'ClickServer_ContadordeAbertura';
    }
    ...
    public function configureView(ViewDataTable $view)
    {
        if (!empty($this->dimension)) {
            $view->config->addTranslations(array('Campaigns' => $this->dimension->getName()));
        }

         $view->config->show_search = false;
         $view->requestConfig->filter_sort_column = 'Campaigns';
    ...
    {
        $view = new View('@ClickServer/getContadordeAbertura');
        $view->myData = array();
